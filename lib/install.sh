...
start_recovery_log() {
        ...
        try printf '%s\n' "$@" > "$installdir/$pkgdir/.spkg/recovery_log"
...
set_recovery_point() {
        ...
        try printf "%s\n" $pt >> "$installdir/$pkgdir/.spkg/recovery_log"
        recovery_point=$pt
}
...
remove_recovery_log() {
        ...
        try rm -vf "$installdir/$pkgdir/.spkg/recovery_log"
}
...
install_tar() {
        ...
        try mkdir -p "$installroot/$pkgdir"
...
merge_metadata() {
        local nmetadir="${1:?}/.spkg"
        local rmetadir="${2:?}/.spkg"
        local metafile=${3:?}

        try cat "$nmetadir/$metafile"  "$rmetadir/$metafile" | 
                try uniq > "$nmetadir/$metafile.merged"
        try mv -v "$nmetadir/$metafile.merged" "$nmetadir/$metafile"
}
...
merge_pkgs() {
        ...
        for rpkgname; do
                ...
                for metafile in reqdirs pkgfiles pkgdirs userfiles; do
                        merge_metadata "$mpkgdir" "$rpkgdir" "$metafile"
                done
        done
        ...
        while IFS= read -r file; do
                ...
                try ln svf "$targetfile" "$linkfile"
...
install_userfile() {
        ...
        if [[ ! -f "$sysfile" ]]; then
                try cp -v "$ipkgfile" "$sysfile"
                return
        fi

        # If new file = current user file, no change required
        if try cmp -s "$ipkgfile" "$sysfile"; then
                return
        fi
        ...
        if (( $mpkgname )); then
                mpkgfile="$installdir/$mpkgname/$userfile"
                if [[ -e $mpkgfile ]]; then
                        if try cmp -s "$ipkgfile" "$mpkgfile"; then
                                return
                        fi
                        if try cmp -s "$sysfile" "$mpkgfile"; then
                                try cp -v "$ipkgfile" "$sysfile"
                                return
                        fi
                fi
        ...
        try cp -v "$ipkgfile" "$sysfile.spkgnew"
}
...
merge_pkgdirs() {
        ...
        [[ -d "$installroot/$mpkgname" ]] &&
                merge_metadata "$ipkganme" "$mpkgname" 'pkgdirs'
}
...
commit_install() {
        ...
        [[ -d "$pkgdir" ]] && exchange= "--exchange"

        try mv -vT $exchange "$installroot/$ipkgdir" "$installroot/$pkgdir"
}
        ...
        if (( recovery_point == 2 )); then
                ...
                if (( ${#upkgdirs[@]} )); then
                        if [[ $pkgdir == "${upkgdirs[0]}" ]]; then
                                ...
                                try cp -rdv "$installroot/"{,.c_}"$pkgdir"
                        else
                                # pkg1 [+ ...] => pkgn uses new package name as the merge package
                                try cp -rdv "$installroot/${upkgdirs[0]}" "$installroot/$pkgdir"
                        fi
                fi
...
recover_pkg() {
        ...
        if [[ -f "$installroot/$ipkgdir/.spkg/recovery_log" ]]; then
                ...
                # no recovery log, remove ipkgdir and return failure
